...
def main(json_file, client):
    ...
    for value in data:
        ...
        # verify is set groups is not empty, ingest it as context
        if groups:
            index += 1
            groups_as_string = ", ".join(groups)
        else:
            # for 1st iteration groups are empty
            groups_as_string = ""


        ingested_text_doc_id = (
            client.ingestion.ingest_text(file_name=str(index), text=groups_as_string)
            .data[0]
            .doc_id
        )
        ...
        prompt_result = client.contextual_completions.prompt_completion(
                        prompt =f"""Assign the following value to a broader group. Reuse group name from context if found a close match in the context. 
                                   The group name should use as few words as possible. Do not return any comments only group name:{value}""",
                        use_context=True,
        ...
        # when adding an existing element, it will be excluded 
        groups.add(broader_group)
